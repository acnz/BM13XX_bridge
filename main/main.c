...
static void fpga_miner_task(void *arg) {
    ESP_LOGI(TAG, "FPGA SPI Miner Task iniciada. A aguardar Jobs...");
    ...
    while (1) {
        if (local_job_version != g_job_version) {
            ...
            uint8_t tx_buf[47] = {0};
            tx_buf[0] = 0x01; // OPCODE
            tx_buf[1] = (uint8_t)(local_job_version & 0xFF); 

            // CÓDIGO CORRIGIDO: Copia os bytes da UART direto pro SPI sem mexer na ordem!
            memcpy(&tx_buf[2], g_midstate_raw, 32);
            memcpy(&tx_buf[34], g_data_raw, 12);
