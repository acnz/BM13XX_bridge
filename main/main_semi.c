...
static void miner_task(void *arg) {
    ...
    while(1) {
        ...
        if (len > 0) {
            // Verifica se é o INÍCIO do cabeçalho do ASIC (0x55 0xAA 0x13)
            if (fpga_rx_buf[0] == 0x55 && fpga_rx_buf[1] == 0xAA && fpga_rx_buf[2] == 0x13) {
                
                // Se a UART fragmentou o pacote (leu menos de 11 bytes), força a leitura do resto!
                while (len < 11) {
                    int extra = uart_read_bytes(FPGA_UART_PORT, fpga_rx_buf + len, 11 - len, 50 / portTICK_PERIOD_MS);
                    if (extra <= 0) break; // Evita loop infinito se a FPGA morrer
                    len += extra;
                }
                
                if (len == 11) {
                    ESP_LOGW(TAG, "ASIC Encontrado pela FPGA! Repassando ao Micro-Stratum...");
                    if (xSemaphoreTake(uart_mutex, portMAX_DELAY)) {
                        uart_write_bytes(SIM_UART_PORT, fpga_rx_buf, 11);
                        xSemaphoreGive(uart_mutex);
                    }
                }
            }
            // Retorno do Nonce da mineração
            else if (len == 4) {
    ...
    while (1) {
        ...
        if (len > 0) {
            ...
            // === ADICIONE ESTE BLOCO ===
                ...
                // Comando INIT3 (Busca de Chip). Repassa EXATAMENTE como chegou para a FPGA
                ESP_LOGW(TAG, "Repassando Handshake (INIT3) para a FPGA...");
                uart_write_bytes(FPGA_UART_PORT, data, len);
            }
            // ===========================
        }
    }
